...
from quai.fetch.base import _select_market_code, _select_index_code, _select_type, _select_bond_market_code
from quai.util.util_date import util_date_stamp, util_date_str2int, util_date_valid, util_tdxtimestamp, util_time_stamp
from quai.util.util_date_trade import util_get_real_datelist, util_future_to_realdatetime, util_get_trade_gap, trade_date_sse
from quai.util.util_log import util_log_info
from quai.util.util_setting import exclude_from_stock_ip_list, future_ip_list, stock_ip_list
from quai.util.util_setting import SETTING
from quai.util.util_parallelism import Parallelism
from quai.util.util_cache import util_cache
...
def __QA_fetch_get_stock_transaction(code, day, retry, api):
    ...
    while cur_offset <= max_offset:
        one_chunk = api.get_history_transaction_data(
            ...
            batch_size, util_date_str2int(day))
        ...
        if len(data_) < 2:
            ...
            data_ = data_.assign(
                ...
                datetime=pd.to_datetime(data_['time'].apply(
                    lambda x: str(day) + ' ' + x)),
                code=str(code))
            data_ = data_.assign(date_stamp=data_['datetime'].apply(lambda x: util_date_stamp(x)),
        ...
        try:
            ...
            data = data.assign(
                ...
                code=str(code), date_stamp=data['datetime'].apply(
                    lambda x: util_date_stamp(str(x)[0:10]))).set_index('date',
                                                                           drop=False,
                                                                           inplace=False)
    ...
    with apix.connect(ip, port):
        ...
        data = data \
            .assign(tradetime=data['datetime'].apply(str), code=str(code),
                ...
                data['datetime'].apply(util_future_to_realdatetime, 1))) \
            .drop(['year', 'month', 'day', 'hour', 'minute'], axis=1,
                  inplace=False) \
...
def __QA_fetch_get_future_transaction(code, day, retry, code_market, apix):
    ...
    while cur_offset <= max_offset:
        one_chunk = apix.get_history_transaction_data(
            code_market, str(code), util_date_str2int(day),
